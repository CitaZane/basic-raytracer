use crate::{camera::Camera, objects::{Hittable, Intersection, Sphere}, ray::Ray, color::Color, image::Image, material::{Scatterable, Material}, point3d::Point3D};
use rand::prelude::*;
...
impl RayTracer{
    pub fn new(image:Image, camera:Camera, objects:Vec<Box< dyn Hittable>>, light: Sphere)->Self{
        Self { 
            image,
            camera,
            objects,
    ...
    pub fn render(&mut self){
        for x in 0..self.image.width{
            eprintln!("Scan lines remaining: {x}");
    ...
    fn find_pixel_color(&self, ray: &Ray, depth: i32)-> Color{
        if depth<= 0 {
            return Color::black();
        }
        ...
        if intersection.is_some(){
            ...
            if light_blocked.is_some(){return  pixel_color}
    ...
    fn hit_scene(&self, ray:&Ray)-> Option<Intersection>{
    ...
    fn cast_ray_to_light(&self, intersection_point: &Point3D) -> Option<Intersection>{
        let light_vec= (self.light.center -  *intersection_point).unit_vector();
        let shadow_ray = Ray::new(intersection_point.clone(), light_vec);
        self.hit_scene(&shadow_ray)
    }
    fn light_specular_diffuse_adjustment(&self,intersection:&Intersection)-> Color{
        const diffuse_coefficient:f64 = 0.4;
        const specular_coefficient:f64 = 0.2;

        let light_vec= (self.light.center -  intersection.point).unit_vector();
        let light_angle = light_vec.dot(&intersection.normal);
        let reflection_vec = light_vec * -2. * light_vec.dot(&intersection.normal) * intersection.normal;
        ...
        let specular = if reflection_angle < 0. {0.}else{reflection_angle};
        let specular_reflection = specular.powf(1.);

        (Color::white() * diffuse_coefficient * diffuse_reflection  + Color::white() * specular_coefficient * specular_reflection) * self.light.material.intensity()
    }
}
